...
import numpy as np
import re
from utils.transform import Random2DTranslation,ColorAugmentation
...
class Data():
    def __init__(self,data="veri",size=(288, 288),sampler="triple",mean=[0.485, 0.456, 0.406],std=[0.229, 0.224, 0.225]):
        ...
        Dataset = {'veri':VeRi,"vehicleid":VehicleID}
        ...
        if sampler==None:
            ...
            self.train_loader = dataloader.DataLoader(self.trainset,sampler=RandomSampler(self.trainset, batch_id=opt.batchid,
                                    ...
                                    pin_memory=True)
        self.test_loader = dataloader.DataLoader(self.testset, batch_size=opt.batchtest, num_workers=4,shuffle = False,  pin_memory=True)
        self.query_loader = dataloader.DataLoader(self.queryset, batch_size=opt.batchtest,num_workers=4, shuffle = False,
                                    pin_memory=True)
class VeRi(dataset.Dataset):
    def __init__(self, transform, dtype, data_path):
        ...
        self.len = 0
        self.data = {'train':'image_train','query':'image_query','test':'image_test'}
