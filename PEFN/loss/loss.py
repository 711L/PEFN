...
class Loss(loss._Loss):
    def __init__(self,lsm=True, margin=1.2):
        super(Loss, self).__init__()
        self.lsm = lsm#label smooth
        self.margin = margin

    def forward(self, outputs, labels):
        num_cls = outputs[1][0].size(-1)#(b,576)
        cross_entropy_loss = CrossEntropyLoss(num_cls, label_smooth=self.lsm)
        triplet_loss = TripletLoss(margin=self.margin)
        if isinstance(outputs[0],(tuple,list)):#前面返回的是一个tuple
